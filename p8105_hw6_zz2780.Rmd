...
I fitted a linear regression model with baby's head circumference, family income, and their interaction term as predictors and baby's birthweight as outcome.
```{r}
model1<-lm(bwt~bhead*fincome, data=data3)
```
...
```{r}
model2<-lm(bwt~blength+gaweeks,data=data3)
```

Model using head circumference, length, sex, and all interactions (including the three-way interaction)
```{r}
model3<-lm(bwt~bhead*blength*babysex, data=data3)

rmse(model3,data=data3)
```
...
cv_df =
  crossv_mc(data3, 100)%>%
  mutate(
    model1=purrr::map(train,~lm(bwt~bhead*fincome, data=.x)),
...
```
The RMSE of each model shows that model 3 is better than the other two models. 
